...
extension Tensor {
  ...
  public func batchGathering<Index: TensorFlowIndex>(
    ...
    // If `axis` is not the first non-batch dimension, transpose so that `axis` becomes
    // `batchDimensionCount`, gather there, then transpose back.
    if axis != batchDimensionCount {
      // Move dimension `axis` up to position `batchDimensionCount`.
      let permutation = Tensor<Int32>(concatenating: [
        ...
        Tensor<Int32>(rangeFrom: Int32(batchDimensionCount),
                      to: Int32(axis), stride: 1, on: device),
      ...
      // Now we transpose back, so that dimensions in `tensor[batchDimensionCount..<axis]`
      // move to just before the `indices` dimensions in `result`.
      ...
      let resultPermutation = Tensor<Int32>(concatenating: [
        ...
        Tensor<Int32>(rangeFrom: Int32(batchDimensionCount),
                      to: Int32(indices.rank), stride: 1, on: device),
    ...
    // If we are already gathering along `batchDimensionCount`, gather directly.

    // Build the "batchIndices" used for indexing. 
    // This was previously enclosed in `withoutDerivative`, but there's no gradient for these ints.
    let batchIndices: Tensor<Index> = {
      var batchIndices = indices
      var accumulated = Tensor<Index>(ones: [], on: device)
      // We multiply up dimensions to get the correct indexing offset.
      for d in (1...batchDimensionCount).reversed() {
        ...
        // Reshape `dIndices` to broadcast across the appropriate dimension(s).
    ...
    let flatTensor = reshaped(
      toShape: innerShape.rankLifted().concatenated(with: outerShape)
    )
    let flatResult = flatTensor.gathering(atIndices: flatIndices)
    return flatResult.reshaped(
      toShape: batchIndices.shapeTensor.concatenated(with: outerShape)
    )
  }
  ...
  @inlinable
  @differentiable(reverse where Scalar: TensorFlowFloatingPoint)
  public func gathering(where mask: Tensor<Bool>, alongAxis axis: Int = 0) -> Tensor<Scalar> {
    precondition(mask.rank != 0, "The boolean mask cannot be a scalar.")
    // Compute the positive axis without needing `withoutDerivative`.
    // If `axis` is negative, wrap around by adding `rank`.
    let posAxis = axis < 0 ? axis + self.rank : axis
    
    // The next few lines are the same as before.
    let leadingSize = shapeTensor[posAxis..<posAxis + mask.rank]
      .product()
      .rankLifted()
...
extension Tensor {
  ...
  @inlinable
  @differentiable(reverse where Scalar: TensorFlowFloatingPoint)
  public func slice(lowerBounds: [Int], upperBounds: [Int]) -> Tensor<Scalar> {
      // TODO: Precondition `lowerBounds.count == upperBounds.count`,
      //       preferably in graph code.
      // Since `upperBounds` and `lowerBounds` are `[Int]`, there's no gradient to block.
      let sizes = zip(upperBounds, lowerBounds).map { $0 - $1 }
