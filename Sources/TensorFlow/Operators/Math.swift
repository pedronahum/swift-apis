...
public func log1mexp<T: TensorFlowFloatingPoint>(_ x: Tensor<T>) -> Tensor<T> {
  // `isTooSmall` is a Bool tensor, so there's no gradient to block for this comparison.
  let isTooSmall = -x .< T(log(2.0))
  
  // Likewise, creating a `ones` tensor does not need to block gradients.
  let ones = Tensor<T>(onesLike: x)
  
  // `xSafe` replaces `x` with `-ones` where `isTooSmall` is true.
  let xSafe = x.replacing(with: -ones, where: isTooSmall)
  
  // The final expression uses the stable approach:
  //  - By default: `log1p(-exp(xSafe))`
  //  - Where `x` is too small: `log(-expm1(x))`
  return log1p(-exp(xSafe))
    .replacing(with: log(-expm1(x)), where: isTooSmall)
}
...
extension Tensor where Scalar: TensorFlowFloatingPoint {
  ...
  @inlinable
  @differentiable(reverse)
  public func logSumExp(squeezingAxes axes: Tensor<Int32>) -> Tensor<Scalar> {
    // 1) Validate the axes.
    ensureValid(axes: axes)
    ...
    // 3) "Offset" is just `rawMax` itself, but with finite values replaced. 
    //    (If you originally intended to block gradients here, see below.)
    let offset = Tensor<Scalar>(zerosLike: rawMax)
      .replacing(with: rawMax, where: rawMax.isFinite)

    // 4) Compute exp(self - offset), reduce-sum, then take log.
    let result = Tensor.log(Tensor.exp(self - offset).sum(squeezingAxes: axes))

    // 5) Reshape `offset` to match the shape of `result` for broadcasting in the add.
    //    If `result.shape` is `[Int]`, `reshaped(to:)` can take `[Int]` directly.
    //    Or if it needs a `Tensor<Int32>`, convert as needed.
    let reshapedOffset = offset.reshaped(like: result)

    // 6) Return the final stabilized log-sum-exp.
    return result + reshapedOffset
  }
  ...
  @inlinable
  @differentiable(reverse)
  public func logSumExp(alongAxes axes: Tensor<Int32>) -> Tensor<Scalar> {
    ensureValid(axes: axes)
    let rawMax = max(alongAxes: axes)
    let offset = Tensor<Scalar>(zerosLike: rawMax)
      .replacing(with: rawMax, where: rawMax.isFinite)
